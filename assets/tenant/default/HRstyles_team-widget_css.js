...
/* The message block hangs under the spinner. .progress-status is already
   position:absolute, so it is the containing block here -- top/left resolve
   against the spinner card rather than some ancestor further up the tree. */
...
/* Sits above the card, centred on the spinner -- deliberately not boxed. */
...
/* One card for every line, rather than a pill per message. The cap leaves
   headroom over the longest current line (~446px including padding) so a small
   copy edit does not start wrapping; anything longer wraps rather than running
   off the toolbar. */
...
/* The card must escape three nested clips to be painted: its own 80px-wide
   toolbar cell, and the 50px-high Webix toolbar containing it. Webix sets
   overflow:hidden on both, so without these the message block renders in the
   DOM at full size and is simply never shown. */
.progress-status-cell,
.progress-status-cell > .webix_template,
.orgchart-teams-toolbar,
.orgchart-teams-toolbar > .webix_scroll_cont {
